...
public class Room
{
		...
		else
		{
			...
			else
			{
				...
				if (flag3)
				{
					...
					if (flag4)
					{
						ReadLog._ReadLog("已加入其他房间", null, true);
						return false;
					}
    ...
    public MsgBase ToMsg_GameRoomInfo()
    {
        MsgGetGameRoomInfo msgGetRoomInfo = new MsgGetGameRoomInfo();
        ...
        foreach (string current in this.playerIds.Keys)
        {
            ...
            ReadLog._ReadLog("Player" + playerInfo.id);
            ReadLog._ReadLog("Player" + playerInfo.name);
            ReadLog._ReadLog("Player" + playerInfo.isOwner);
            ReadLog._ReadLog("Player" + playerInfo.roomid);

            num++;
        }
        ReadLog._ReadLog("Player" + msgGetRoomInfo.players.Length);
        msgGetRoomInfo.serverdescription = describe;
	...
	public bool StartBattle()
	{
		...
		else
		{
			...
			msgEnterBattle.humans = new HumanInfo[this.playerIds.Count];
